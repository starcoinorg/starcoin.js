...
import {
  encodeScriptFunction, generateRawUserTransaction, getSignedUserTransaction, encodeStructTypeTags, encodeScriptFunctionByResolve
} from "./tx";
import { createMultiEd25519KeyShard, dec2bin, bin2dec, setBit, isSetBit } from "./multi-sign";
import { showMultiEd25519Account } from "./account";
import { bool } from '../lib/runtime/serde';
import * as starcoin_types from "../lib/runtime/starcoin_types";

test('MultiEd25519KeyShard', async () => {
  // 2-3 multi-sig
  const publicKeys = [
  ...
  console.log({ rtx })
  const signatures = await shard.sign(rtx)
