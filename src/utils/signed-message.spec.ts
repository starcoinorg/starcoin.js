...
const msgHex2 = Buffer.from(exampleMessage, 'utf8').toString('hex')
console.log({ msgHex2 })

const msgHex3 = Buffer.from(msgHex2, 'utf8').toString('hex')
console.log({ msgHex3 })

const msgBytes3 = new Uint8Array(Buffer.from(msgHex2, 'utf8'))

const rawMessageHex = '30783638363536633663366637373666373236633634'
// const rawMessageBytes = signedMessage.message.message
// const rawMessageHex = hexlify(rawMessageBytes)
const rawMessage = Buffer.from((rawMessageHex), 'hex').toString('utf8')
...
test('encode SignedMessage: simple-keyring', async () => {
  ...
  const signedMessageHex = await encodeSignedMessage(msgBytes3, privateKeyBytes, chainId);
